...
import com.nubits.android_wallet.ui.preference.TrustedServer.ServerStatus;
import com.nubits.nubitsj.store.ValidHashStore.TrustedServersInterface;
...
public class TrustedServerList extends ArrayList<TrustedServer> implements TrustedServersInterface {
    ...
    /**
     * Inserts the Daio default server into the list
     */
    public void insertDaio() {
        ...
        dbExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                final TrustedServer server = TrustedServersDatabaseHelper.getInstance(context).insertDaio();
    ...
    public boolean noDaio() {

        for (TrustedServer server : this)
            if (server.url.equals(TrustedServersDatabaseHelper.DEFAULT_SERVER_DAIO))
                return false;
